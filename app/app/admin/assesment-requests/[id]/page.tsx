...
import AssessmentStatusUpdatePanel from '@/components/admin/removal/AssessmentStatusUpdatePanel';
import AssessmentAdminNotesPanel from '@/components/admin/removal/AssessmentAdminNotesPanel';
import CopyToRemovalButton from '@/components/admin/removal/CopyToRemovalButton';
import {
  ArrowLeft,
  Mail,
  Phone,
...
export async function generateMetadata({
  ...
  return {
    title: `Assessment Request #${id.slice(0, 8)}`,
    description:
      'Admin: View and update this free assessment request. Copy to removal request when ready.',
    openGraph: {
      title: `Assessment Request | Admin | Reach Them AI`,
      description: 'View and manage this assessment request.',
      images: ['/featured.png'],
      type: 'website',
    },
  };
...
export default async function AdminAssessmentRequestDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  // assessment_requests may not be in generated Database types — use generic client for query
  type GenericClient = import('@supabase/supabase-js').SupabaseClient;
  const { data: request, error } = await (adminClient as unknown as GenericClient)
  ...
  // Generate signed URLs for evidence files (same bucket as removal evidence)
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div className="mb-6">
        <Button asChild variant="ghost" size="sm">
          <Link href="/app/admin/removal-requests?tab=assessment">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to Free Assessment Requests
          </Link>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <DetailSection title="Contact Info">
            <div className="space-y-2">
              ...
              {request.contact_phone && (
                <DetailRow icon={Phone} value={request.contact_phone} />
              )}
            </div>
          </DetailSection>
          ...
          <DetailSection title="Review Details">
            <div className="space-y-3">
              <DetailRow
                icon={Globe}
                value={
                  PLATFORM_LABELS[request.platform ?? ''] ?? request.platform ?? 'N/A'
                }
              />
              {request.review_author && (
                <DetailRow icon={UserIcon} value={request.review_author} />
              )}
              {request.review_url && (
                <a
          ...
          <DetailSection title="Removal Reason">
            <div className="space-y-2">
              <DetailRow
                icon={AlertTriangle}
                value={
                  REASON_LABELS[request.removal_reason ?? ''] ??
        ...
        <div className="space-y-6">
          <AssessmentStatusUpdatePanel
            requestId={request.id}
            currentStatus={request.status}
            adminUserId={user.id}
          />
          <AssessmentAdminNotesPanel
            requestId={request.id}
            initialNotes={request.admin_notes ?? ''}
          />
          <CopyToRemovalButton assessmentId={request.id} />
        </div>
      </div>
    </div>
  );
