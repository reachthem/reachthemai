...
function LoginForm() {
    ...
    useEffect(() => {
        if (searchParams.get('registered') !== '1' || registerToastShown.current) return;
        registerToastShown.current = true;
        toast.success('Your account was created. You can sign in now.');
        const qs = new URLSearchParams(searchParams.toString());
        qs.delete('registered');
        const tail = qs.toString();
        router.replace(tail ? `/auth/login?${tail}` : '/auth/login');
    }, [searchParams, router]);
    ...
    return (
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            ...
            <div className="mt-6 text-center text-sm">
                ...
                {' '}
                <Link
                    href={redirectTo !== '/app' ? `/auth/register?redirect=${encodeURIComponent(redirectTo)}` : '/auth/register'}
...
export default function LoginPage() {
    return (
        <Suspense fallback={<div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 animate-pulse" />}>
            <LoginForm />
        </Suspense>
    );
}