...
export class SassClient {
    ...
    async getMyTodoList(page: number = 1, pageSize: number = 100, order: string = 'created_at', done: boolean | null = false) {
        let query = (this.client as import('@supabase/supabase-js').SupabaseClient).from('todo_list').select('*').range(page * pageSize - pageSize, page * pageSize - 1).order(order)
        if (done !== null) {
    ...
    async createTask(row: Record<string, unknown>) {
        return (this.client as import('@supabase/supabase-js').SupabaseClient).from('todo_list').insert(row)
    }

    async removeTask (id: number) {
        return (this.client as import('@supabase/supabase-js').SupabaseClient).from('todo_list').delete().eq('id', id)
    }

    async updateAsDone (id: number) {
        return (this.client as import('@supabase/supabase-js').SupabaseClient).from('todo_list').update({done: true}).eq('id', id)
    }
