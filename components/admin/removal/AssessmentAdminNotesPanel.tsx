...
import { updateAssessmentAdminNotes } from '@/app/actions/admin-assessment-requests';
import { Loader2 } from 'lucide-react';

interface AssessmentAdminNotesPanelProps {
  requestId: string;
  initialNotes: string;
}

export default function AssessmentAdminNotesPanel({
  requestId,
  initialNotes,
}: AssessmentAdminNotesPanelProps) {
  ...
  const saveNotes = useCallback(
    async (value: string) => {
      ...
      setError(null);
      try {
        const result = await updateAssessmentAdminNotes(requestId, value);
        setSavedAt(
